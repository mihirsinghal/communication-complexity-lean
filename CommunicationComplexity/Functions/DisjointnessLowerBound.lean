...
/-- The event `X_T = false` has probability `1 / 2`. -/
theorem measureReal_specialX_false :
    (volume (((specialX n) ⁻¹' {false}) : Set (HardSample n))).toReal = (1 / 2 : ℝ) := by
  have hset :
      ((specialX n) ⁻¹' {false} : Set (HardSample n)) =
        specialBitsEvent n false false ∪ specialBitsEvent n false true := by
    ext ω
    rcases ω with ⟨T, xT, yT, other⟩
    cases xT <;> cases yT <;> simp [specialX, specialBitsEvent]
  have hdisj : Disjoint (specialBitsEvent n false false) (specialBitsEvent n false true) := by
    rw [Set.disjoint_left]
    intro ω hfalse htrue
    have hyFalse : ω.yT = false := by
      simpa [specialBitsEvent] using hfalse.2
    have hyTrue : ω.yT = true := by
      simpa [specialBitsEvent] using htrue.2
    rw [hyFalse] at hyTrue
    simp at hyTrue
  change (volume : Measure (HardSample n)).real ((specialX n) ⁻¹' {false}) = (1 / 2 : ℝ)
  rw [hset, measureReal_union hdisj MeasurableSet.of_discrete]
  rw [show (volume : Measure (HardSample n)).real (specialBitsEvent n false false) =
      (1 / 4 : ℝ) by
    simpa [Measure.real] using measureReal_specialBitsEvent n false false]
  rw [show (volume : Measure (HardSample n)).real (specialBitsEvent n false true) =
      (1 / 4 : ℝ) by
    simpa [Measure.real] using measureReal_specialBitsEvent n false true]
  norm_num
...
/-- If Alice's special bit is `false`, the generated inputs are disjoint. -/
theorem mem_disjointEvent_of_specialX_eq_false
    {ω : HardSample n} (hX : specialX n ω = false) :
    ω ∈ disjointEvent n := by
  change Disjoint (X n ω) (Y n ω)
  rw [disjoint_X_Y_iff]
  intro hboth
  rw [show ω.xT = false by simpa [specialX] using hX] at hboth
  simp at hboth
...
/-- Under the disjoint-conditioned hard distribution, `X_T=false` has mass `2 / 3`. -/
theorem disjointCondMeasure_measureReal_specialX_false :
    (disjointCondMeasure n).real (((specialX n) ⁻¹' {false}) : Set (HardSample n)) =
      (2 / 3 : ℝ) := by
  rw [disjointCondMeasure]
  rw [ProbabilityTheory.cond_real_apply MeasurableSet.of_discrete]
  have hsubset : ((specialX n) ⁻¹' {false} : Set (HardSample n)) ⊆ disjointEvent n := by
    intro ω hω
    exact mem_disjointEvent_of_specialX_eq_false n (by simpa using hω)
  have hinter :
      disjointEvent n ∩ ((specialX n) ⁻¹' {false} : Set (HardSample n)) =
        (specialX n) ⁻¹' {false} := by
    exact Set.inter_eq_right.mpr hsubset
  rw [hinter]
  rw [show (volume : Measure (HardSample n)).real (disjointEvent n) = (3 / 4 : ℝ) by
    simpa [Measure.real] using measureReal_disjointEvent n]
  rw [show (volume : Measure (HardSample n)).real
      (((specialX n) ⁻¹' {false}) : Set (HardSample n)) = (1 / 2 : ℝ) by
    simpa [Measure.real] using measureReal_specialX_false n]
  norm_num
...
open Classical in
/-- Symmetric reweighting step for Bob: conditioning on `(X_T,Y_T)=(0,0)` costs at most a factor
`2` relative to conditioning only on `X_T=false` under the disjoint distribution. -/
theorem volume_cond_specialZeroZero_measureReal_le_two_mul_disjointSpecialXFalseMeasure
    (S : Set (HardSample n)) :
    ((volume : Measure (HardSample n))[|specialZeroZero n]).real S ≤
      2 * (disjointSpecialXFalseMeasure n).real S := by
  let μ : Measure (HardSample n) := disjointCondMeasure n
  let X0 : Set (HardSample n) := (specialX n) ⁻¹' {false}
  have hzero_subset_x0 : specialZeroZero n ⊆ X0 := by
    intro ω hω
    rw [specialZeroZero, specialBitsEvent] at hω
    simpa [X0, specialX] using hω.1
  have hcond_zero :
      (volume : Measure (HardSample n))[|specialZeroZero n] = μ[|specialZeroZero n] := by
    simpa [μ] using
      volume_cond_eq_disjointCondMeasure_cond_of_subset_disjointEvent n
  ...
  have hleft :
    ...
    rw [ProbabilityTheory.cond_real_apply MeasurableSet.of_discrete]
  have hright :
      (disjointSpecialXFalseMeasure n).real S =
        (μ.real X0)⁻¹ * μ.real (X0 ∩ S) := by
    rw [disjointSpecialXFalseMeasure]
    change (μ[|X0]).real S = (μ.real X0)⁻¹ * μ.real (X0 ∩ S)
    rw [ProbabilityTheory.cond_real_apply MeasurableSet.of_discrete]
  have hzero_mass : μ.real (specialZeroZero n) = (1 / 3 : ℝ) := by
    simpa [μ] using disjointCondMeasure_measureReal_specialZeroZero n
  have hx0_mass : μ.real X0 = (2 / 3 : ℝ) := by
    simpa [μ, X0] using disjointCondMeasure_measureReal_specialX_false n
  have hmono : μ.real (specialZeroZero n ∩ S) ≤ μ.real (X0 ∩ S) :=
    measureReal_mono (by
      intro ω hω
      exact ⟨hzero_subset_x0 hω.1, hω.2⟩)
  rw [hleft, hright, hzero_mass, hx0_mass]
  norm_num
  linarith
...
open Classical in
/-- Bob's one-bit fiber KL cost on a `Z=z` fiber, set to `0` on zero-mass fibers. -/
...
open Classical in
/-- Pointwise Pinsker for Bob, with zero-mass `Z` fibers handled by `yFiberKL`. -/
...
open Classical in
/-- Integrated Bob Pinsker bound over the `D ∧ X_T=false` conditioned measure. -/
theorem two_mul_integral_yDistance_sq_le_integral_yFiberKL_disjointSpecialXFalse
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool) :
    2 * (∫ ω, (yDistance n p (zVariable n p ω)) ^ 2
        ∂(disjointSpecialXFalseMeasure n)) ≤
      ∫ ω, yFiberKL n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) := by
  let μ : Measure (HardSample n) := disjointSpecialXFalseMeasure n
  haveI : IsProbabilityMeasure μ := by
    simpa [μ] using disjointSpecialXFalseMeasure_isProbabilityMeasure n
  have hpoint :
      ∀ ω : HardSample n,
...
open Classical in
/-- Jensen's inequality converts a squared Bob-side average-distance estimate under
`D ∧ X_T=false` into the average-distance estimate used before Markov. -/
theorem disjointSpecialXFalseMeasure_integral_yDistance_le_of_integral_sq_le
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool)
    {γ : ℝ}
    (hsq :
      ∫ ω, (yDistance n p (zVariable n p ω)) ^ 2 ∂(disjointSpecialXFalseMeasure n) ≤
        γ ^ 4) :
    ∫ ω, yDistance n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) ≤
      γ ^ 2 := by
  let μ : ProbabilityMeasure (HardSample n) :=
    ⟨disjointSpecialXFalseMeasure n, disjointSpecialXFalseMeasure_isProbabilityMeasure n⟩
  let f : HardSample n → ℝ := fun ω => yDistance n p (zVariable n p ω)
  have hjensen :
      (∫ ω, f ω ∂(μ : Measure (HardSample n))) ^ 2 ≤
        ∫ ω, (f ω) ^ 2 ∂(μ : Measure (HardSample n)) :=
    FiniteMeasureSpace.probabilityMeasure_sq_integral_le_integral_sq μ f
...
open Classical in
/-- Markov's inequality converts the textbook Bob-side average-distance estimate under
`D ∧ X_T=false` into a bad-`Z` probability bound. -/
theorem disjointSpecialXFalseMeasure_yDistance_bad_le_of_integral_le
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool)
    {γ : ℝ}
    (hγ : 0 < γ)
    (havg :
      ∫ ω, yDistance n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) ≤ γ ^ 2) :
    (disjointSpecialXFalseMeasure n).real
      {ω | γ < yDistance n p (zVariable n p ω)} ≤ γ := by
  let μ : Measure (HardSample n) := disjointSpecialXFalseMeasure n
  let f : HardSample n → ℝ := fun ω => yDistance n p (zVariable n p ω)
  haveI : IsProbabilityMeasure μ := by
    simpa [μ] using disjointSpecialXFalseMeasure_isProbabilityMeasure n
  have hmarkov :
    ...
    mul_meas_ge_le_integral_of_nonneg
      (μ := μ) (f := f)
      (ae_of_all _ fun ω => yDistance_nonneg n p (zVariable n p ω))
      Integrable.of_finite γ
  have hbad_le :
      μ.real {ω : HardSample n | γ < f ω} ≤ μ.real {ω : HardSample n | γ ≤ f ω} :=
    measureReal_mono (by
...
theorem claim621_y_fiberKL_disjointSpecialXFalse_le_three_halves_bobInfoTerm
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool) :
    ∫ ω, yFiberKL n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) ≤
      (3 / 2 : ℝ) * bobInfoTerm n p := by
  sorry

open Classical in
/-- Bob marginal part of Claim 6.21, symmetric to the Alice marginal estimate. -/
theorem claim621_y_fiberKL_disjointSpecialXFalse_le
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool)
    {γ : ℝ}
    (hγ : 0 < γ)
    (hinfo : claimInfo n p ≤ 2 * γ ^ 4 / 3) :
    ∫ ω, yFiberKL n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) ≤
      2 * γ ^ 4 := by
  have hkl :=
    claim621_y_fiberKL_disjointSpecialXFalse_le_three_halves_bobInfoTerm n p
  have hbob : bobInfoTerm n p ≤ claimInfo n p :=
    bobInfoTerm_le_claimInfo n p
  have hclaim :
      (3 / 2 : ℝ) * bobInfoTerm n p ≤ (3 / 2 : ℝ) * claimInfo n p :=
    mul_le_mul_of_nonneg_left hbob (by norm_num)
  have hsmall : (3 / 2 : ℝ) * claimInfo n p ≤ γ ^ 4 := by
    have hmul := mul_le_mul_of_nonneg_left hinfo (by norm_num : (0 : ℝ) ≤ 3 / 2)
    nlinarith
  have hγ4_nonneg : 0 ≤ γ ^ 4 := by positivity
...
open Classical in
/-- Bob marginal part of Claim 6.21, symmetric to the Alice marginal estimate. -/
theorem claim621_y_average_sq_distance_disjointSpecialXFalse_le
    (p : Deterministic.Protocol (Set (Fin n)) (Set (Fin n)) Bool)
    {γ : ℝ}
    (hγ : 0 < γ)
    (hinfo : claimInfo n p ≤ 2 * γ ^ 4 / 3) :
    ∫ ω, (yDistance n p (zVariable n p ω)) ^ 2 ∂(disjointSpecialXFalseMeasure n) ≤
      γ ^ 4 := by
  have hpinsker :=
    two_mul_integral_yDistance_sq_le_integral_yFiberKL_disjointSpecialXFalse n p
  have hkl := claim621_y_fiberKL_disjointSpecialXFalse_le n p hγ hinfo
  nlinarith
...
theorem claim621_y_bad_on_specialZeroZero_le
  ...
  have hsquared :=
    claim621_y_average_sq_distance_disjointSpecialXFalse_le n p hγ hinfo
  have havg :
      ∫ ω, yDistance n p (zVariable n p ω) ∂(disjointSpecialXFalseMeasure n) ≤
        γ ^ 2 :=
    disjointSpecialXFalseMeasure_integral_yDistance_le_of_integral_sq_le n p hsquared
  have hbad_x0 :
      (disjointSpecialXFalseMeasure n).real badY ≤ γ := by
    simpa [badY] using
      disjointSpecialXFalseMeasure_yDistance_bad_le_of_integral_le n p hγ havg
  have htransfer :
      (μ[|specialZeroZero n]).real badY ≤ 2 * γ := by
    have h :=
      volume_cond_specialZeroZero_measureReal_le_two_mul_disjointSpecialXFalseMeasure n badY
    have hbad_x0_two :
        2 * (disjointSpecialXFalseMeasure n).real badY ≤ 2 * γ := by
      nlinarith
    exact h.trans (by simpa [μ] using hbad_x0_two)
  have hcond_eq :
      (μ[|specialZeroZero n]).real badY =
        (μ.real (specialZeroZero n))⁻¹ * μ.real (specialZeroZero n ∩ badY) := by
    rw [ProbabilityTheory.cond_real_apply MeasurableSet.of_discrete]
  have hA : μ.real (specialZeroZero n) = (1 / 4 : ℝ) := by
    simpa [μ, Measure.real] using measureReal_specialZeroZero n
  change μ.real (specialZeroZero n ∩ badY) ≤ γ / 2
  rw [hcond_eq, hA] at htransfer
  norm_num at htransfer
  linarith
