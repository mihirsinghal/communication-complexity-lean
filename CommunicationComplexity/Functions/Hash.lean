import CommunicationComplexity.Basic
import CommunicationComplexity.FiniteProbabilitySpace
import Mathlib.Probability.UniformOn

namespace CommunicationComplexity

open MeasureTheory ProbabilityTheory

namespace Functions.Hash

/-- A hash function on `α` with outputs in `Fin k`. -/
abbrev HashSpace (α : Type*) (k : ℕ) := α → Fin k

noncomputable instance hashRange.measureSpace (k : ℕ) :
    MeasureSpace (Fin k) :=
  ⟨ProbabilityTheory.uniformOn Set.univ⟩

noncomputable instance hashRange.isProbabilityMeasure (k : ℕ) [NeZero k] :
    IsProbabilityMeasure (volume : Measure (Fin k)) := by
  letI : Nonempty (Fin k) := ⟨⟨0, Nat.pos_of_neZero k⟩⟩
  change IsProbabilityMeasure (ProbabilityTheory.uniformOn Set.univ)
  infer_instance

noncomputable instance hashRange.finiteProbabilitySpace (k : ℕ) [NeZero k] :
    FiniteProbabilitySpace (Fin k) :=
  FiniteProbabilitySpace.of (Fin k)

noncomputable instance hashSpace.finiteProbabilitySpace
    (α : Type*) [Fintype α] (k : ℕ) [NeZero k] :
    FiniteProbabilitySpace (HashSpace α k) := by
  infer_instance

/-- For distinct inputs, a uniformly random hash collides with probability `1 / k`. -/
theorem collision_prob_le
    (α : Type*) [Fintype α]
    (k : ℕ) [NeZero k] (x y : α) (hxy : x ≠ y) :
    (volume {h : HashSpace α k | h x = h y}).toReal ≤ (1 : ℝ) / k := by
  classical
  let q := Fin k
  let collisionPiece : q → Set (HashSpace α k) := fun a =>
    Set.pi Set.univ
      (Function.update
        (Function.update (fun _ : α => (Set.univ : Set q)) x ({a} : Set q))
        y
        ({a} : Set q))
  -- Split the collision event according to the common hash value.
  have hcollision :
      {h : HashSpace α k | h x = h y} = ⋃ a : q, collisionPiece a := by
    ext h
    constructor
    · intro hh
      refine Set.mem_iUnion.2 ?_
      refine ⟨h x, ?_⟩
      intro i
      by_cases hi : i = y
      · subst hi
        simpa [Function.update] using hh.symm
      · by_cases hx : i = x
        · subst hx
          simp [Function.update]
        · simp [Function.update, hi, hx]
    · intro hh
      rcases Set.mem_iUnion.1 hh with ⟨a, ha⟩
      have hx' : h x = a := by
        simpa [collisionPiece, Set.mem_pi, hxy, Function.update] using ha x
      have hy' : h y = a := by
        simpa [collisionPiece, Set.mem_pi, hxy, Function.update] using ha y
      exact hx'.trans hy'.symm
  -- Different common hash values give disjoint pieces.
  have hdisjoint : Pairwise fun a b => Disjoint (collisionPiece a) (collisionPiece b) := by
    intro a b hab
    refine Set.disjoint_left.2 ?_
    intro h ha hb
    have hx' : h x = a := by
      simpa [collisionPiece, Set.mem_pi, hxy, Function.update] using ha x
    have hx'' : h x = b := by
      simpa [collisionPiece, Set.mem_pi, hxy, Function.update] using hb x
    exact hab (hx'.symm.trans hx'')
  have hmeas : ∀ a : q, MeasurableSet (collisionPiece a) := by
    intro a
    exact MeasurableSet.of_discrete
  have hsingle (a : q) :
      volume ({a} : Set q) = (1 : ENNReal) / k := by
    change ProbabilityTheory.uniformOn Set.univ ({a} : Set q) = (1 : ENNReal) / k
    rw [ProbabilityTheory.uniformOn_univ]
    simp [q]
  have hpiece_measure (a : q) :
      volume (collisionPiece a) = ((1 : ENNReal) / k) ^ 2 := by
    have hyx : y ≠ x := fun hyx => hxy hyx.symm
    dsimp [collisionPiece]
    change Measure.pi (fun _ : α => (volume : Measure q))
        (Set.pi Set.univ
          (Function.update
            (Function.update (fun _ : α => (Set.univ : Set q)) x ({a} : Set q))
            y
            ({a} : Set q))) =
      ((1 : ENNReal) / k) ^ 2
    rw [Measure.pi_pi]
    have hy_mem : y ∈ (Finset.univ : Finset α).erase x := by
      simp [Finset.mem_erase, hyx]
    rw [← Finset.prod_erase_mul (s := Finset.univ)
      (f := fun z : α =>
        volume
          (Function.update
            (Function.update (fun _ : α => (Set.univ : Set q)) x ({a} : Set q))
            y
            ({a} : Set q) z))
      (a := x) (by simp)]
    rw [← Finset.prod_erase_mul (s := (Finset.univ : Finset α).erase x)
      (f := fun z : α =>
        volume
          (Function.update
            (Function.update (fun _ : α => (Set.univ : Set q)) x ({a} : Set q))
            y
            ({a} : Set q) z))
      (a := y) hy_mem]
    have hrest :
        ∏ z ∈ ((Finset.univ : Finset α).erase x).erase y,
          volume
            (Function.update
              (Function.update (fun _ : α => (Set.univ : Set q)) x ({a} : Set q))
              y
              ({a} : Set q) z) = 1 := by
      refine Finset.prod_eq_one ?_
      intro z hz
      have hz_ne_y : z ≠ y := (Finset.mem_erase.1 hz).1
      have hz_ne_x : z ≠ x := by
        exact (Finset.mem_erase.1 (Finset.mem_of_mem_erase hz)).1
      simp [Function.update, hz_ne_x, hz_ne_y]
    rw [hrest]
    simp [Function.update, hsingle, pow_two]
  -- Each piece has probability `1 / k^2`, and there are `k` of them.
  rw [hcollision, measure_iUnion hdisjoint hmeas, tsum_fintype]
  rw [ENNReal.toReal_sum]
  · have hsum :
        ∑ a : q, (volume (collisionPiece a)).toReal =
          ∑ a : q, (((1 : ENNReal) / k) ^ 2).toReal := by
      refine Finset.sum_congr rfl ?_
      intro a ha
      rw [hpiece_measure]
    rw [hsum]
    have hconst :
        (((1 : ENNReal) / k) ^ 2).toReal = ((1 : ℝ) / k) ^ 2 := by
      rw [ENNReal.toReal_pow, ENNReal.toReal_div]
      simp
    rw [hconst, Finset.sum_const, nsmul_eq_mul]
    have hcard : ((((Finset.univ : Finset q).card : ℕ) : ℝ)) = k := by
      simp [q]
    rw [hcard]
    have hk_ne : (k : ℝ) ≠ 0 := by
      have hk_pos : (0 : ℝ) < k := by exact_mod_cast (Nat.pos_of_neZero k)
      linarith
    have hsum_real : (k : ℝ) * ((1 : ℝ) / k) ^ 2 = (1 : ℝ) / k := by
      field_simp [pow_two, hk_ne]
    exact le_of_eq hsum_real
  · intro a ha
    rw [hpiece_measure]
    have hk_ne : k ≠ 0 := NeZero.ne k
    simp [hk_ne]

end Functions.Hash

end CommunicationComplexity
